//this is the mainClass used in the Ethiopian Calendar
...
int a=-1;
try{
a=Integer.parseInt(ll.getText());
}
catch(Exception ex){}


boolean eventFound=false; // found a date with an event
for(toDo tt : adl.todolist)
if(yearChanging==tt.year && monthChanging==tt.month-1 && tt.date==a){
eventFound=true;
break; }


if((yearChanging==todayEthio[0] && monthChanging==todayEthio[1] && a==todayEthio[2])&&eventFound ){
c.setBackground(new Color(230, 180, 220));}
else if(yearChanging==todayEthio[0] && monthChanging==todayEthio[1] && a==todayEthio[2]){
c.setBackground(new Color(190, 190 , 255));
}
else if(eventFound)
	c.setBackground(new Color(190, 250, 200));
...
public Component getTableCellRendererComponent (JTable tab, Object value, boolean selected, boolean focused, int row, int column){
if(value!=null)
if(Integer.parseInt(value.toString())==todayEthio[2] && yearChanging==todayEthio[0] && monthChanging==todayEthio[1])
