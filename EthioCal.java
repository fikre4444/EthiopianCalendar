...
JMenuItem deleteEvent = new JMenuItem("Delete Events of One Day");

clr.addActionListener(new ClearAll());
adl = new AddListener(todayEthio[0]);
addEvent.addActionListener(adl);
deleteEvent.addActionListener(new DeleteEventListener(adl.todolist));
menu.add(addEvent);
menu.add(displayAll);
...
panelCenter.setBackground(new Color(100, 55, 55));
JScrollPane jss = new JScrollPane(panelCenter);
panelC.add(jss, BorderLayout.CENTER);
...
JScrollPane jsss = new JScrollPane(panelcc);
panelcc.setLayout(new BoxLayout(panelcc, BoxLayout.Y_AXIS));
panelAll.add(jsss, BorderLayout.CENTER);
panelcc.add(Box.createVerticalStrut(10));
panelcc.setBackground(new Color(100, 55, 55));
...
//deletes one day event by selecting the list of dates
